...
export default  function AddDetailsForm() {
  ...
  const [userProfile, setUserProfile] = useState({ id:"",email: '', member:{} });
...
.catch(error => {
  ...
  const onSubmit = async ( values: z.infer<typeof memberSchema>) =>
  {
    ...
    if (response.ok) {
      ...
      router.push("/member-dashboard/my-details")
    } else {
      const errorData = await response.json();
      setError(errorData)
      console.log('Error adding your details:', errorData);
  ...
  return (   
    <div className="max-w-xl mx-auto p-8 text-xs">
      <p className="text-base font-bold mb-6 text-slate-600">
        Add your Details,for account:
